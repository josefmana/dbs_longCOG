...
for ( i in 1:imp ) {
  ...
  for ( j in tests ) {
    ...
    # scale in the jth imputed data set
    if ( j %in% c( paste0("cs_tmt_",c("a","b")), paste0("cs_pst_",c("d","w","c")), paste0("cs_staix",1:2) ) ) {
...
f <- list(
  m1_rhs_doms = paste0( "drs | cens(cens_drs) ~ 1 + ", paste("time", doms, sep = " * ", collapse = " + "), " + (1 + time | id)" ) %>% as.formula() %>% bf(),
  m2_rhs_tests = paste0( "drs | cens(cens_drs) ~ 1 + ", paste("time", tests, sep = " * ", collapse = " + "), " + (1 + time | id)" ) %>% as.formula() %>% bf()
)
...
for ( i in names(f) ) p[[i]] <- c(
  ...
  prior( horseshoe(df = 3, scale_global = 1), class = b ),
  # random effects
