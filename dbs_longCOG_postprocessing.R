...
sapply (
  ...
  pivot_longer( # change to a long format
    rownames( loadings(efa[[1]][[nf-2]]) ) , values_to = "Loading" , names_to = "Test"
  ) %>%
  mutate( t.order = rep(1:(nrow(t.s2)-2), nf ) ) %>% # prepare an order variable to sort factors
  # plot it
